...
#' }
#'
loadWorkbook <- function(file, xlsxFile = NULL, isUnzipped = FALSE, na.convert = FALSE) {
  ...
  ## core
  if (length(coreXML) == 1) {
    coreXML <- paste(openxlsx:::readUTF8(coreXML), collapse = "")
    wb$core <- openxlsx:::removeHeadTag(x = coreXML)
  }
  ...
  if (length(workbookRelsXML) > 0) {
    workbookRelsXML <- paste(openxlsx:::readUTF8(workbookRelsXML), collapse = "")
  ...
  if (length(chartSheetsXML) > 0) {
    ...
    chartSheetRIds <- unlist(openxlsx:::getId(workbookRelsXML))
  ...
  if (length(workbookXML) > 0) {
    workbook <- openxlsx:::readUTF8(workbookXML)
    workbook <- openxlsx:::removeHeadTag(workbook)
    ...
    sheetrId <- unlist(openxlsx:::getRId(sheets))
...
# #active sheet -----------------------------------------------------------
    ...
    for (i in seq_along(sheetrId)) {
      if (is_chart_sheet[i]) {
        ...
        txt <- paste(openxlsx:::readUTF8(chartSheetsXML[j]), collapse = "")
        ...
        tabColour <- openxlsx:::getChildlessNode(xml = txt, tag = "tabColor")
    ...
    calcPr <- openxlsx:::getChildlessNode(xml = workbook, tag = "calcPr")
    if (length(calcPr) > 0) {
      wb$workbook$calcPr <- calcPr
    }

    ## additional workbook attributes
    extLst <- openxlsx:::getChildlessNode(xml = workbook, tag = "extLst")
    if (length(extLst) > 0) {
      wb$workbook$extLst <- extLst
    }

    workbookPr <- openxlsx:::getChildlessNode(xml = workbook, tag = "workbookPr")
    if (length(workbookPr) > 0) {
      wb$workbook$workbookPr <- workbookPr
    }

    workbookProtection <- openxlsx:::getChildlessNode(xml = workbook, tag = "workbookProtection")
    ...
    ## defined Names
    dNames <- openxlsx:::getNodes(xml = workbook, tagIn = "<definedNames>")
    if (length(dNames) > 0) {
      dNames <- gsub("^<definedNames>|</definedNames>$", "", dNames)
      wb$workbook$definedNames <- paste0(openxlsx:::getNodes(xml = dNames, tagIn = "<definedName"), ">")
    }
  }
  ...
  if (length(pivotTableXML) > 0) {
    ...
    if (length(pivotTableRelsXML) > 0) {
      wb$pivotTables.xml.rels <- unlist(lapply(pivotTableRelsXML, function(x) openxlsx:::removeHeadTag(cppReadFile(x))))
    }
    ...
    caches <- openxlsx:::getNodes(xml = workbook, tagIn = "<pivotCaches>")
    caches <- openxlsx:::getChildlessNode(xml = caches, tag = "pivotCache")
  ...
  if (length(themeXML) > 0) {
    wb$theme <- openxlsx:::removeHeadTag(paste(unlist(lapply(sort(themeXML)[[1]], openxlsx:::readUTF8)), collapse = ""))
  }


  ## externalLinks
  if (length(extLinksXML) > 0) {
    wb$externalLinks <- lapply(sort(extLinksXML), function(x) openxlsx:::removeHeadTag(cppReadFile(x)))
  ...
  if (length(extLinksRelsXML) > 0) {
    wb$externalLinksRels <- lapply(sort(extLinksRelsXML), function(x) openxlsx:::removeHeadTag(cppReadFile(x)))
  }
  ...
  file_rIds <- unlist(openxlsx:::getId(worksheet_rId_mapping))
  file_names <- file_names[match(sheetrId, file_rIds)]

  worksheetsXML <- file.path(dirname(worksheetsXML), file_names)
  wb <- openxlsx:::loadworksheets(wb = wb, styleObjects = styleObjects, xmlFiles = worksheetsXML, is_chart_sheet = is_chart_sheet)
  ...
  if (length(sheetRelsXML) > 0) {
    ...
    xml <- lapply(seq_along(allRels), function(i) {
      if (haveRels[i]) {
        xml <- openxlsx:::readUTF8(allRels[[i]])
        xml <- openxlsx:::removeHeadTag(xml)
        xml <- gsub("<Relationships .*?>", "", xml)
        xml <- gsub("</Relationships>", "", xml)
        xml <- openxlsx:::getChildlessNode(xml = xml, tag = "Relationship")
      } else {
        xml <- "<Relationship >"
      }
      return(xml)
    ...
    if (length(slicerCachesXML) > 0) {
      ...
      wb$slicerCaches <- sapply(slicerCachesXML[order(nchar(slicerCachesXML), slicerCachesXML)], function(x) openxlsx:::removeHeadTag(cppReadFile(x)))
    ...
    if (length(tablesXML) > 0) {
      ...
      if (length(unlist(tables)) > 0) {
        ...
        wb$tables <- sapply(tablesXML, function(x) openxlsx:::removeHeadTag(paste(openxlsx:::readUTF8(x), collapse = "")))
    ...
    if (length(drawingRelsXML) > 0) {
      dRels <- lapply(drawingRelsXML, openxlsx:::readUTF8)
      dRels <- unlist(lapply(dRels, openxlsx:::removeHeadTag))
      dRels <- gsub("<Relationships .*?>", "", dRels)
      dRels <- gsub("</Relationships>", "", dRels)
    }

    if (length(drawingsXML) > 0) {
      dXML <- lapply(drawingsXML, openxlsx:::readUTF8)
      dXML <- unlist(lapply(dXML, openxlsx:::removeHeadTag))
      ...
      if (any(hasDrawing)) {
        for (i in seq_along(xml)) {
          if (hasDrawing[i]) {
            ...
            if (any(ind)) {
              txt <- paste(openxlsx:::readUTF8(vmlDrawingXML[ind]), collapse = "\n")
              txt <- openxlsx:::removeHeadTag(txt)
      ...
      for (i in seq_along(xml)) {
        if (hasComment[i]) {
          ...
          if (any(ind)) {
            ...
            cd <- unique(openxlsx:::getNodes(xml = txt, tagIn = "<x:ClientData"))
            ...
            txt <- paste(openxlsx:::readUTF8(grep(target, commentsXML, value = TRUE)), collapse = "\n")
            txt <- openxlsx:::removeHeadTag(txt)

            authors <- openxlsx:::getNodes(xml = txt, tagIn = "<author>")
            authors <- gsub("<author>|</author>", "", authors)

            comments <- openxlsx:::getNodes(xml = txt, tagIn = "<commentList>")
            comments <- gsub("<commentList>", "", comments)
            comments <- openxlsx:::getNodes(xml = comments, tagIn = "<comment")
            ...
            style <- lapply(comments, openxlsx:::getNodes, tagIn = "<rPr>")
    ...
    if (any(hasDrawing)) {
      for (i in seq_along(xml)) {
        if (hasDrawing[i]) {
          image_ids <- unlist(openxlsx:::getId(drawXMLrelationship[[i]]))
  ...
  for (i in 1:nSheets) {
    wb$worksheets[[i]]$hyperlinks <- openxlsx:::xml_to_hyperlink(wb$worksheets[[i]]$hyperlinks)
  }
  ...
  if (length(queryTablesXML) > 0) {
    ...
    wb$queryTables <- unlist(lapply(queryTablesXML[order(ids)], function(x) openxlsx:::removeHeadTag(cppReadFile(xmlFile = x))))
    wb$Content_Types <- c(
      wb$Content_Types,
  ...
  if (length(connectionsXML) > 0) {
    wb$connections <- openxlsx:::removeHeadTag(cppReadFile(xmlFile = connectionsXML))
  ...
  if (length(tableRelsXML) > 0) {
    ...
    xml <- sapply(xml, openxlsx:::removeHeadTag, USE.NAMES = FALSE)
  ...
  wb$ActiveSheet <- as.integer(openxlsx:::getAttrs(activesheet,"activeTab")$activeTab) + 1L
